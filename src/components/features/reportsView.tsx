...
const Reports = ({route}) => {
  ...
  console.log('typetypetype', userDetails);
  ...
  console.log('userReportListOPD--->', userReportListOPD);
  ...
  useEffect(() => {
    const userReportsData = async (userId: any) => {
      ...
      const userReports = await getUserReports(searchedUserId, userId, type);
      console.log('userReports--->', userReports);
      setUserReportListCurrent(userReports?.data?.current);
      setUserReportListHistory(userReports?.data?.history);
      setUserReportListOPD(userReports?.data?.opd);
